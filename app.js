...
async function loadModels() {
    // Use relative path from current HTML location, not root, to work on file:// and nested hosting
...
async function start() {
    await loadModels();
    if (!video) return; // Guard for pages without camera
    await setupCamera();
    console.log("Camera and models are ready.");
}

// Only start when on pages that include the register UI
if (document.getElementById('register')) {
    start().catch((err) => console.error('Init error', err));
}

const registerButton = document.getElementById('register');
const nameInput = document.getElementById('name');

if (registerButton && nameInput) {
